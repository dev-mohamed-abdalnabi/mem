...
export const dataService = {
  ...
  uploadMemeFile: async (file: File, bucket: string = "memes", onProgress?: (pct: number) => void): Promise<string> => {
    ...
    // الموبايل الحديثة (4K/HEVC مثلاً) بتبقى كبيرة بطبيعتها قبل ما نضغطها تحت.
    // ملحوظة مهمة: الحد ده كان اتقلل قبل كده لـ 50 ميجا على فكرة إنه "حد منطقي"،
    // لكن فيديو كاميرا موبايل حديث بسهولة بيتعدى 50 ميجا حتى لدقيقة أو دقيقتين -
    // يعني كان بيترفض هنا *قبل ما يوصل لخطوة الضغط أصلاً*، وده كان بيلغي فايدة
    // الضغط بالكامل بالظبط في الحالة اللي هو معمول عشانها (فيديوهات كاميرا
    // كبيرة). رفعناه لحد يسمح للضغط إنه ياخد فرصته، والحد الحقيقي اللي بيتحكم
    // في الحجم النهائي هو maxUploadSize (30 ميجا) بعد الضغط تحت.
    const maxOriginalSize = file.type.startsWith("video/") ? 300 * 1024 * 1024 : 15 * 1024 * 1024;
