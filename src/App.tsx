...
export default function App() { 
  ...
  // Supabase Authentications Modal
  ...
  // Edit Profile Modal State (الجديد لتشغيل زرار التعديل)
  ...
  // دالة حفظ التعديلات على الملف الشخصي
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 font-sans" dir="rtl">
      ...
      {/* FB Style Lightbox - شاشة عرض الصور زي فيسبوك بالظبط */}
      {lightboxImage && (
        <div 
          className="fixed inset-0 z-[9999] bg-black flex flex-col animate-fade-in"
        >
          <div className="flex justify-between items-center p-4 bg-gradient-to-b from-black/80 to-transparent absolute top-0 w-full z-10">
            <button 
              className="text-white p-2 hover:bg-white/20 rounded-full transition-colors cursor-pointer"
          ...
          <div 
            className="flex-1 flex items-center justify-center p-0 w-full h-full cursor-pointer"
            onClick={() => setLightboxImage(null)}
          >
            <img 
              src={lightboxImage} 
      ...
      {/* Edit Profile Modal - نافذة تعديل الملف الشخصي */}
      {showEditModal && (
          ...
          <div className="bg-white dark:bg-gray-800 rounded-3xl max-w-md w-full p-6 text-right border border-gray-200 dark:border-gray-700 shadow-2xl relative animate-fade-in">
            <div className="flex justify-between items-center mb-6 border-b border-gray-100 dark:border-gray-700 pb-3">
              <h3 className="font-extrabold text-lg flex items-center gap-2">
                <Edit2 className="w-5 h-5 text-blue-500" /> تعديل الملف الشخصي
              </h3>
              <button onClick={() => setShowEditModal(false)} className="text-gray-400 hover:text-gray-900 dark:hover:text-white bg-gray-100 dark:bg-gray-700 p-1.5 rounded-full">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleSaveProfileEdit} className="flex flex-col gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 focus:ring-2 focus:ring-blue-500 rounded-xl px-4 py-3 text-sm font-bold text-gray-900 dark:text-white"
                />
              </div>
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 focus:ring-2 focus:ring-blue-500 rounded-xl px-4 py-3 text-sm text-gray-900 dark:text-white resize-none"
                  placeholder="اكتب نبذة عنك..."
                />
              </div>
              <button
                type="submit" disabled={editLoading || !editUsername.trim()}
                className="w-full mt-2 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-500 text-white font-black py-3 rounded-2xl text-sm flex justify-center gap-2 items-center transition-all"
              >
      ...
      <Header
        ...
        onSearch={setSearchQuery} activeTab={activeTab} onUserSwitch={handleUserSwitch}
        availableProfiles={profiles} onMarkNotificationsRead={async () => { await dataService.markNotificationsAsRead(currentUser.id); setNotifications(prev => prev.map(n => ({...n, is_read: true}))); }}
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 pb-24 md:pb-6 w-full flex-1 flex gap-6" onClick={handleGlobalImageClick}>
            ...
            <div className="flex flex-col gap-2.5">
              {profiles.slice(0, 3).map((prof, index) => (
                <div key={prof.id} className="flex items-center gap-2.5 border-b border-gray-100 dark:border-gray-700 pb-2 last:border-0 last:pb-0">
                  ...
                  <img src={prof.avatar_url || 'https://api.dicebear.com/7.x/bottts/svg'} alt="" className="w-8 h-8 rounded-lg object-cover cursor-pointer" onClick={() => setLightboxImage(prof.avatar_url)} />
        ...
        <div className="flex-1 max-w-full md:max-w-2xl order-2">
          ...
          {activeTab === "feed" && (
            <div className="flex flex-col gap-4">
              {loading ? <div className="text-center py-12 text-gray-400 font-bold">جاري تحميل الإفيهات...</div> : filteredMemes.length === 0 ? (
                <div className="bg-white dark:bg-gray-800 rounded-2xl p-12 text-center text-gray-400 font-extrabold">مفيش ميمز تطابق استعلامك!</div>
              ) : (
          ...
          {/* PROFILE - REDESIGNED FB STYLE WITH WORKING EDIT BUTTON */}
                    ...
                    <div className="px-6 relative flex flex-col sm:flex-row items-center sm:items-end justify-between -mt-16 sm:-mt-12 pb-4 border-b border-gray-100 dark:border-gray-700 gap-4">
                      <div className="flex flex-col sm:flex-row items-center sm:items-end gap-4 sm:gap-6 w-full">
                        <div className="relative group shrink-0">
                          <div 
                            className="w-32 h-32 sm:w-36 sm:h-36 rounded-full border-4 border-white dark:border-gray-800 bg-gray-200 shadow-lg overflow-hidden cursor-pointer bg-white"
                      ...
                      {/* زرار تعديل الملف شغال والوانه متناسقة في الداكن */}
                      <div className="flex gap-2 sm:mb-2 w-full sm:w-auto justify-center shrink-0">
                        {!isOwnProfile && isRealUser ? (
                          ...
                          <button 
                            ...
                            className="bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-800 dark:text-white px-5 py-2.5 rounded-xl text-sm font-black flex items-center gap-2 transition-all w-full sm:w-auto justify-center shadow-sm"
                          >
          ...
          {/* الباقي زي ما هو مفيش تغييرات جوهرية فيه */}
        </div>
      ...
      {showAuthModal && (
        <div className="fixed inset-0 z-[100] bg-slate-900/80 backdrop-blur-md flex items-center justify-center p-4 shadow-2xl" dir="rtl">
           {/* ... كود فورم تسجيل الدخول لم يتغير ... */}
           <div className="bg-white dark:bg-gray-800 rounded-3xl max-w-md w-full p-6 text-right border border-gray-200 dark:border-gray-700 shadow-2xl relative animate-fade-in">
            ...
            <div className="flex bg-gray-100 dark:bg-gray-900 p-1 rounded-2xl mb-4">
              <button onClick={() => { setAuthTab("signin"); setAuthError(""); setAuthSuccess(""); }} className={`flex-1 text-center py-2 text-[11px] font-black rounded-xl cursor-pointer transition-all ${authTab === "signin" ? "bg-white dark:bg-gray-700 text-blue-600 dark:text-blue-400 shadow-sm" : "text-gray-500"}`}>تسجيل الدخول 👋</button>
              <button onClick={() => { setAuthTab("signup"); setAuthError(""); setAuthSuccess(""); }} className={`flex-1 text-center py-2 text-[11px] font-black rounded-xl cursor-pointer transition-all ${authTab === "signup" ? "bg-white dark:bg-gray-700 text-blue-600 dark:text-blue-400 shadow-sm" : "text-gray-500"}`}>إنشاء حساب 🚀</button>
            </div>
